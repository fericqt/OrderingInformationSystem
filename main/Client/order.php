...
<body>
            ...
            <table id="items-table1" class="table table-bordered">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>";
                        echo '<a ' . ($row['Payment'] != 0 ? 'hidden ' : '') . 'type="button" class="btn btn-primary edit-btn" data-toggle="modal" data-target="#edit-item" data-id="' . $row['OrderID'] . '">Process Payment</a>';
    ...
    <div class="modal fade" id="edit-item" tabindex="-1" role="dialog" aria-labelledby="edit-item-label" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="edit-item-label">Process Payment</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="edit-item-form" method="post">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="EditIdTrack" name="IdTrack" readony required>
                        </div>
                        <div class="form-group">
                            <label for="EditTotalCost">Total Cost:</label>
                            <input type="text" class="form-control" id="EditTotalCost" name="TotalCost" readonly required>
                        </div>
                        <div class="form-group">
                            <label for="EditPayment">Payment:</label>
                            <input type="number" step="0.01" class="form-control" id="EditPayment" name="Payment" required>
                        </div>
                        <div class="form-group">
                            <label for="EditChange">Change:</label>
                            <input type="text" class="form-control" id="EditChange" name="Change" readonly required>
                        </div>
                        <div class="text-right">
                            <button type="button" class="btn btn-secondary mr-2" data-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-primary">Update Payement</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        $('#edit-item-form').on('submit', function(e) {
            ...
            $.ajax({
                url: '../../db_function/editOrderPayment.php',
                type: 'POST',
                data: formData,
                success: function(response) {
                    if (response === '1') {
                        ...
                        alert("There was an error encountered during processing of the payment. Please try again later!");
                    }
                },
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
        ...
        $('#EditPayment').on('input', function() {
            var total = $('#EditTotalCost').val();
            var payment = $('#EditPayment').val();
            $('#EditChange').val(payment - total);
        });
